...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    for other in game_state["others"]:
        free_space[tuple(other[1])] = False
    ...
    while len(frontier) > 0:
        ...
        if len(found_targets) == 0:
            POI_position = game_state["self"][1]
    ...
    dist = POI_position - np.array(game_state["self"][1])
    ...
    if game_state["others"] != []:
        ...
        for other in game_state["others"]:
            manhattan = np.sum(np.abs(np.array(other[1]) - np.array(game_state["self"][1])))
            if manhattan <= manhattan_min:
                nearest_enemy = other
                manhattan_min = manhattan
        dist = other[1] - np.array(game_state["self"][1])
    ...
    bomb_left = int(game_state["self"][0])
    # channels = []
    # channels.append(...)
